...
# TODO: divide gi() functions by step
...
#   image(x, y, ra)
  ...
  res = data.frame(solve = slv(A, b, P=SSOR.precond.approach1(A, .8))$root, 
                  answer = get.res.vector(n, I, J, x, y))
  
  View(res)
}

form.A.matrix <- function(n, h1, h2, I){
  ...
  A[diag.num == 1+(I-2)] = A[diag.num == -1-(I-2)] = rep(1/h2^2, times=length(A[diag.num == 1+(I-2)]))
...
form.b <- function(n, I, J, x, y, h1, h2, f, g1, g2, g3, g4){
  ...
  for (j in 2:(J-1))
    for (i in 2:(I-1)){
      k = (I-2)*(j-2) + (i-2) + 1
      b[k] = -f(x[i], y[j])
      if (j == 2) b[k] = b[k] - g3(x[i])
      if (j == (J-1)) b[k] = b[k] - g4(x[i])
      if (i == 2) b[k] = b[k] - g1(y[i])
      if (i == (I-1)) b[k] = b[k] - g2(y[i])
    }
